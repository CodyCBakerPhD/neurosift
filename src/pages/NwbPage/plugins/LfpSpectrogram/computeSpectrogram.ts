...
// Compute a short-time Fourier transform (spectrogram) of a 1D signal.
// Returns power spectral density in dB for a stack of overlapping Hann-windowed
// segments. The frequency axis is one-sided (0 .. Nyquist).
export const computeSpectrogram = (
  input: SpectrogramInput,
): SpectrogramResult => {
  const { signal, samplingFrequency, signalStartTimeSec, windowSize } = input;
  ...
  const numWindows =
    signal.length >= windowSize
      ? 1 + Math.floor((signal.length - windowSize) / step)
      : 0;
